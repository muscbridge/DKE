...
internal_flag = 1;      % if set to 1, dke will output tensors, e'vecs, and e'vals, as well as d2/k2, d3/k3, color fa, and white matter model-derived maps
version_str = 'Diffusional Kurtosis Estimator (DKE) version 2.5.1; December 2012\n';
...
if nargin ~= 1
    fprintf('\nUsage: dke paramsfile\n')
    fprintf('\nparamsfile  DKE processing parameters file\n\n')
    return
end
...
fn_out_struc.kt = 'kt';                             % kurtosis tensor
fn_out_struc.dt = 'dt';                             % DKI diffusion tensor
...
for i = 1:length(id)
    ...
    subject_list{i} = ['CON' sprintf('%03d', id(i))];

end
studydir = 'W:\Travis\DKI_TLE\MTLE_connectivity_20130821';

for isubject = 1:length(subject_list)

    dir_subj = fullfile(studydir, subject_list{isubject});      % subject root folder

    fn_gradients = fullfile(dir_subj, [subject_list{isubject} '_bvec.dat']);
    
    if ~exist(fn_params, 'file')
        fprintf('\nInput parameters file %s does not exist!\n\n', fn_params)
        return
    end
    ...
    file = textread(fn_params,'%s','delimiter','\n','whitespace','');
    for i = 1:length(file)
        eval(file{i})
    end
...
%        dke_preprocess_dicom(dir_subj, fn_params);
%        fn_subject_img_prefix = fullfile(dir_subj, 'DKI_comb', 'rdki');
    ...
    dke_estimate(fn_subject_img_prefix, idx_1st_img, bval, ndir, Kmin, NKmax, Kmin_final, Kmax_final, T, find_brain_mask_flag, dki_method, dti_method, fwhm_img, fn_subject_noise, fwhm_noise, fn_gradients, idx_gradients, fn_out_struc_subject, median_filter_method, map_interpolation_method, internal_flag)
