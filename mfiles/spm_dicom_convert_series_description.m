function out = spm_dicom_convert(hdr,opts,root_dir,format, series_description, extra_b0)
...
if (strcmp(opts,'all') || strcmp(opts,'mosaic')) && ~isempty(mosaic),
    fmos = convert_mosaic(mosaic, root_dir, format, series_description, extra_b0);
end;
if (strcmp(opts,'all') || strcmp(opts,'standard')) && ~isempty(standard),
    fstd = convert_standard(standard,root_dir,format, series_description, extra_b0);
end;
...
%_______________________________________________________________________
function fnames = convert_mosaic(hdr, root_dir, format, series_description, extra_b0)
...
for i=1:length(hdr),
    ...
    % Output filename
    %-------------------------------------------------------------------
    fnames{i} = getfilelocation(hdr{i}, series_description, extra_b0);
    if isempty(fnames{i})
        continue
    end
...
%_______________________________________________________________________
function fnames = convert_standard(hdr,root_dir,format, series_description, extra_b0)
hdr = sort_into_volumes(hdr);
fnames = cell(length(hdr),1);
for i=1:length(hdr),
    fnames{i} = write_volume(hdr{i},root_dir,format, series_description, extra_b0);
end;
return;
%_______________________________________________________________________
...
%_______________________________________________________________________
function fname = write_volume(hdr,root_dir,format, series_description, extra_b0)

% Output filename
%-------------------------------------------------------------------
fname = getfilelocation(hdr{1}, series_description, extra_b0);
if isempty(fname)
    return
end
...
function fname = getfilelocation(hdr, series_description, extra_b0)

fname = nii_name(hdr, series_description, extra_b0);
...
function fn = nii_name(hdr, series_description, extra_b0)

hdr.SeriesDescription = deblank(hdr.SeriesDescription);
series_description = deblank(series_description);

if sum(strcmpi(hdr.SeriesDescription, series_description))  % if one of the diffusion series
    
    if extra_b0 == 1 && strcmpi(hdr.SeriesDescription, series_description{end})     % if b = 0
        
        suffix = ['0_' num2str(hdr.SeriesNumber) '_' num2str(hdr.AcquisitionNumber)];
        fn = fullfile(pwd, 'intermediate_processing', 'dki_b0_nii', ['dki_' suffix '.nii']);
        
    else    % otherwise (dki series)
        ...
        elseif ~isempty(strfind(hdr.SequenceName, 'ep'))
            ...
            if isempty(bval_end)    % if '#' was not found, assumed that DWI was acquired for only one direction (transversal)
                ...
                trace_weighted_flag = 1;
            else
                trace_weighted_flag = 0;
            end
            bval = hdr.SequenceName(bval_start:bval_end);
            if trace_weighted_flag == 0
                gdir = deblank(hdr.SequenceName(bval_end+2:end));
            else
                gdir = '1';
            end
            suffix = [bval '_' gdir];
        ...
        idx = find(strcmpi(hdr.SeriesDescription, series_description) ~= 0);
        fn = fullfile(pwd, 'intermediate_processing', ['dki_avg' num2str(idx) '_nii'], ['dki_' suffix '.nii']);
