...
%GET ODF FUNCTION IN SPHERICAL FORM

fK = @(x,A)-(1./((sin(x(:,1)).*cos(x(:,2))).^2.*A(23)+(sin(x(:,1)).*sin(x(:,2))).^2.*A(24)+cos(x(:,1)).^2.*A(25)+2.*(sin(x(:,1)).*cos(x(:,2))).*(sin(x(:,1)).*sin(x(:,2))).*A(26)+...
...
%GET ODF FUNCTION IN CARTESIAN FORM

fK2 = @(x,A)-(1./((x(:,1)).^2.*A(23)+(x(:,2)).^2.*A(24)+x(:,3).^2.*A(25)+2.*(x(:,1)).*(x(:,2)).*A(26)+...
