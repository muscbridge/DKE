...
for isubject = 1:length(input_struc.subject_list)
    ...
    hdr=spm_vol(fullfile(subj_dir,[subj_name 'fa.nii'])); % header template   
    ...
    if input_struc.odf_optimization
        load(fullfile(subj_dir,[subj_name 'DT.mat']));    %Diffusion tensor (DT)
        load(fullfile(subj_dir,[subj_name 'KT.mat']));    %Kurtosis tensor (KT)
    ...
    if input_struc.tractography_flg
        ...
        vox_to_ras = hdr.mat([permute_img 4],:)*[diag(invert_img) double(invert_img==-1)'.*hdr.dim(permute_img)'+input_struc.shift*[1;1;1];0 0 0 1];
...
clear DT DTN KT KTN subj_dir subj_name fn_diary fid fn_out_struc_subject ...
    ...
    fK A DKI_P max_idx DKI_V DKI_max idx chi_i degi DTI_V L Li x
